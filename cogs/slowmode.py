...
class Slowmode(commands.Cog):
    ...
    async def slowmode(self,ctx, seconds: int):
        if seconds <= 21600 and seconds > 0:
            embed = discord.Embed(
                Title=f'Slowmode ', color=0xff0000)

            embed.add_field(
                name=f'Slowmode Changed !',value=f'Slowmode is now set to {seconds} seconds'
            )
            embed.set_image(url='https://media1.tenor.com/images/27b559e217424b733741a34f4a5c24c6/tenor.gif')
    ...
    async def resetslow(self,ctx):
        ...
        embed.add_field(
            name=f'Slowmode Changed !',value=f'Slowmode is now set to 0'
            )
