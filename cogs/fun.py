...
class Fun(commands.Cog):
    ...
    async def _8ball(self, ctx, *, question):
        ...
        embed = discord.Embed(
            timestamp=ctx.message.created_at, title="8ball", color=0xFF0000
        )
        embed.add_field(
            name=f"Question: {question}", value=f"Reply: {random.choice(responses)}"
        )
    ...
    async def sendnuke(self, ctx, member: discord.Member):
        ...
        embed.add_field(
            name=f"{ctx.author} killed {member.name}", value="Misson Accomplished"
        )
        embed.set_image(
    ...
    async def say(self, ctx, tit, *, desc):
        if len(tit) > 60 or len(desc) > 80:
            ...
            embed = discord.Embed(title=tit, description=desc, color=0x1ABC9C)
            await ctx.send(embed=embed)
