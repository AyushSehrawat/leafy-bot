...
class NewInfo(commands.Cog):
    ...
    async def server(self, ctx):
        ...
        embed = discord.Embed(
            ...
            title='<:info2:799169805747224606> Server Info', color=0xff0000)
        ...
        embed.add_field(
            name='<a:earth:799175818285809706> Region',
            value=f'{reg.capitalize()}')
        embed.add_field(
        ...
        embed.add_field(
            name='<:channels:799230731397890049> Text Channels',
            value=f'{txt}')
        embed.add_field(
            name='<:channels:799230731397890049> Voice Channels',
            value=f'{vc}')
        embed.add_field(
            name='<a:prefix:799189793589821461> Prefix',
            value=f'`{pre}`',
            inline=False)
        embed.set_footer(
    ...
    async def bot(self, ctx):
        ...
        embed = discord.Embed(
            timestamp=ctx.message.created_at,
            title=':robot:  Bot Info', color=0xff0000)
        embed.set_thumbnail(url=self.client.user.avatar_url)
        embed.add_field(
            name='<:dotty:799513382113640468> Helping', value=f'{ser} servers')
        embed.add_field(
            name='<a:devhello:799514685610917908> Serving',
            value=f'{mem} members')
        embed.add_field(
            name='<a:prefix:799189793589821461> Prefix', value=f'`{pre}`')
        embed.add_field(
        ...
        embed.add_field(
            name='<:minilight:799521289026142249> Made By',
            value='Mini.py#5183')
        embed.set_footer(
            text=f'Requested By: {ctx.author.name}',
            icon_url=f'{ctx.author.avatar_url}')
    ...
    async def user(self,ctx,member:discord.Member=None):
        ...
        embed.add_field(
            name='<a:happyblob:793040737267744768> Nickname',
            value=f'{member.nick}'
        )
        embed.add_field(
            name=':credit_card: Id',
        ...
        embed.add_field(
            name=':flower_playing_cards: Joined Discord',
            value=f'{c}'
        )
        embed.add_field(
            name='<a:devcheck:799178026729734166> Joined Server',
            value=f'{j}'
        )
        embed.add_field(
