...
class Mod(commands.Cog):
    ...
    async def ban(self,
        ...
        if member == self.client.user:
            ...
            embed = discord.Embed(
                title="Ban",
                description=
                f"{member.name} has been banned by {ctx.author.name}",
                color=0xff000)
    ...
    async def clear(self, ctx, amount=4):
        if amount > 200:
            await ctx.send(
                '**Currently due to bot hosting we support only 200 limit**')
        else:
        ...
        if member == self.client.user:
            ...
            if member.top_role < ctx.author.top_role:
                ...
                embed = discord.Embed(
                    title="Kick",
                    description=
                    f"{member.name} has been kicked by {ctx.author.name}",
                    color=0xff000)
                await ctx.send(embed=embed)

    @commands.command(
        description=
        "Softbans a member ( Unban after ban to clear chat , to clear chat )")
    @commands.has_permissions(kick_members=True)
    async def softban(self, ctx, member: discord.Member):
        ...
        if member == self.client.user:
            ...
            embed = discord.Embed(
                title="SoftBan",
                description=
                f"{member.name} has been softbanned by {ctx.author.name}",
                color=0xff000)
            await ctx.send(embed=embed)
    ...
    @commands.command(
        description="Adds the mentioned to role to mentioned/id memebr")
    @commands.has_permissions(manage_roles=True)
    async def addrole(self, ctx, member: discord.Member,
        ...
        if guild.me.top_role < member.top_role:
            ...
            embed.set_image(
                url=
                "https://media.discordapp.net/attachments/791915442259230740/794196807185989632/tenor.gif"
            )
            await ctx.send(embed=embed)

    @commands.command(
        description="Removed the mentioned role from mentioned user/id")
    @commands.has_permissions(manage_roles=True)
    ...
    @commands.command(
        description="Blocks a user from sending message in that channel")
    @commands.has_permissions(kick_members=True)
        ...
        if member == self.client.user:
            ...
            if member.permissions_in(ctx.channel).send_messages == False:
                ...
                embed = discord.Embed(
                    title="Block",
                    description=
                    f"{member.mention} is blocked by {ctx.author.name}",
                    color=0xff000)
                await ctx.send(embed=embed)
    ...
    async def unblock(self, ctx, member: discord.Member):
        if member.permissions_in(ctx.channel).send_messages == True:
            ...
            embed = discord.Embed(
                title="Unblock",
                description=
                f"{member.mention} is unblocked by {ctx.author.name}",
                color=0xff000)
            await ctx.send(embed=embed)
    ...
    async def nuke(self,ctx, channel_name):
        ...
        if existing_channel:
            ...
            embed = discord.Embed(
                title=f'Channel nuked || {ctx.author.name} || ',
                color = 0x00ffff
            )
            embed.set_image(
