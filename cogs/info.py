...
class Info(commands.Cog):
    ...
    async def spoon(self, ctx):
        embed = discord.Embed(
            title="Spoonfeeding", description=None, color=0xff0000)
        embed.add_field(
            name='Not here',
            value=
            "Spoon feeding is considered bad practice here as it can make the learner dependent on others, and programming isn't just writing code but also figuring out the logic and stuff for a given process. Something you don't do when you are spoon fed. In short terms, people learn from trial and error."
        )
    ...
    async def ping(self, ctx):
        ...
        embed.set_author(
            name="Latency",
            icon_url=
            "https://store-images.s-microsoft.com/image/apps.24298.14073362953807348.18acabda-19d7-4bf5-8056-352b7495ecef.fc31be50-f27e-4c81-9b14-18c06c40e837?mode=scale&q=90&h=270&w=270&background=%23FFFFFF"
        )
    ...
    @commands.command(
        aliases=["mem"], description="Shows total member in the server")
    ...
    async def vote(self, ctx):
        embed = discord.Embed(
            title='Vote leafy !',
            description=
            "**[Vote Here](https://voidbots.net/bot/791888515100573727/)**",
            color=0xff0000)
        await ctx.send(embed=embed)
