...
class Warn(commands.Cog):
            ...
            if guild.me.top_role < member.top_role:
                ...
                if len(reason) > 60:
                    ...
                    if member == self.client.user:
                        ...
                        newuser = {
                            "id": member.id,
                            "Cases": [[passwor, reason, ctx.author.mention, warns]],
                            "warns": warns,
                        }
    ...
    @commands.command(description="Shows your warns")
    ...
    async def case(self, ctx, member: discord.Member):
        ...
        if stats is None:
            ...
            total_warns = stats["warns"]
            embed = discord.Embed(title=f"Case - {member.name}", color=0xFF0000)
            ...
            embed.add_field(name="Total warns", value=f"{total_warns}", inline=False)
            embed.set_thumbnail(url=member.avatar_url)
            await ctx.send(embed=embed)

    @commands.command(
        aliases=["cw"], description="Clears the data of mentioned user / id"
    )
    @commands.has_permissions(ban_members=True)
